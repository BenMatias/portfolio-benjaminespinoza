const translations = {
  ...
  <li><strong>Service Delivery Optimization</strong>: Analyzed customer service data alongside technical and commercial teams to improve response times and customer satisfaction.</li>
    ...
    cs_usm_matrix_title: "User Story Map: Visual Product Matrix",
    cs_usm_matrix_desc: "3D User flow matrix organized by <strong>Main Activities (Backbone)</strong>, <strong>Flow Steps (User Steps)</strong> and <strong>Detailed Stories (Details)</strong>:",
    cs_usm_act1: "1. CMS Autonomy & No-Code Publishing",
    cs_usm_act2: "2. Telemetry & Digital Analytics",
    cs_usm_act3: "3. QA Functional Quality & Deployment",
    cs_usm_st11: "1.1 UI Selection",
    cs_usm_st12: "1.2 Drag & Drop Edit",
    cs_usm_st13: "1.3 Pub. Strapi 5",
    cs_usm_st21: "2.1 GTM Events",
    cs_usm_st22: "2.2 GA4 Event Log",
    cs_usm_st23: "2.3 Clarity Heatmaps",
    cs_usm_st31: "3.1 Staging Deploy",
    cs_usm_st32: "3.2 Form QA Matrix",
    cs_usm_st33: "3.3 Prod Release",
    cs_usm_dt11: "UX-approved responsive visual component library.",
    cs_usm_dt12: "Puck CMS visual editor with live preview.",
    cs_usm_dt13: "Strapi 5 Headless API updates pages without dev redeploy.",
    cs_usm_dt21: "DataLayer & triggers in GTM Debugger (`click_apply_now`).",
    cs_usm_dt22: "Conversion metrics and custom dimensions in GA4.",
    cs_usm_dt23: "Anonymized mobile session recordings.",
    cs_usm_dt31: "Automated deployment to Staging environment.",
    cs_usm_dt32: "15-point functional testing matrix.",
    cs_usm_dt33: "QA sign-off document & production release milestone.",
    ...
    cs_usm_b101_desc: "<strong>As a Product Owner</strong>, I want to build and publish dynamic landing pages in Puck CMS connected to Strapi 5 without depending on developer sprints.",
    cs_usm_b101_ac: "<strong>Acceptance Criteria (BDD):</strong><br>• <i>Given:</i> User is authenticated in Puck CMS visual editor.<br>• <i>When:</i> Drags corporate components and clicks 'Publish to Strapi 5'.<br>• <i>Then:</i> The page deploys live in production in under 15 minutes.",
    cs_usm_b102_desc: "<strong>As a UX Designer & PO</strong>, I want to validate wireframe mockups created with Firebase AI before passing them to Strapi 5.",
    cs_usm_b102_ac: "<strong>Acceptance Criteria (BDD):</strong><br>• <i>Given:</i> The Firebase AI mockup has approval from commercial stakeholders.<br>• <i>When:</i> Design tokens are exported to the CMS.<br>• <i>Then:</i> Components preview 100% responsive on mobile and desktop.",
    cs_usm_b201_desc: "<strong>As a Business Analyst</strong>, I want to configure triggers in GTM to capture application intent in GA4.",
    cs_usm_b201_ac: "<strong>Acceptance Criteria (BDD):</strong><br>• <i>Given:</i> User navigates on the public product landing page.<br>• <i>When:</i> Clicks on the primary CTA `click_apply_now`.<br>• <i>Then:</i> GTM fires the event to GA4 recording session and device category.",
    cs_usm_b202_desc: "<strong>As a Product Owner</strong>, I want to integrate Microsoft Clarity to record anonymized sessions and detect mobile friction.",
    cs_usm_b202_ac: "<strong>Acceptance Criteria (BDD):</strong><br>• <i>Given:</i> Clarity script is active in GTM global tag.<br>• <i>When:</i> Users interact on smartphones.<br>• <i>Then:</i> Weekly heatmaps generate identifying dead clicks.",
    cs_usm_b301_desc: "<strong>As a Business Analyst</strong>, I want to execute the QA Functional Testing matrix in Staging prior to sign-off.",
    cs_usm_b301_ac: "<strong>Acceptance Criteria (BDD):</strong><br>• <i>Given:</i> Landing created in Puck CMS is published in Staging.<br>• <i>When:</i> 100% of form test case matrix passes.<br>• <i>Then:</i> QA functional certification document is signed to enable release.",
    cs_usm_b302_desc: "<strong>As a Product Owner</strong>, I want a blocking gate preventing production release if blocking bugs are detected.",
    cs_usm_b302_ac: "<strong>Acceptance Criteria (BDD):</strong><br>• <i>Given:</i> A functional test case fails in QA matrix.<br>• <i>When:</i> Issue is logged in Jira with High severity.<br>• <i>Then:</i> Pipeline holds deployment and alerts the Agile triad.",
  ...
  <li><strong>Optimización de Tiempos de Respuesta</strong>: Análisis de datos de servicio y clientes junto a equipos técnicos y comerciales para mejorar los tiempos de atención y respuesta a clientes.</li>
    ...
    cs_usm_matrix_desc: "Matriz tridimensional del flujo de usuario organizada por <strong>Actividades Principales (Backbone)</strong>, <strong>Pasos de Flujo (User Steps)</strong> y <strong>Historias Detalladas (Details)</strong>:",
    cs_usm_act1: "1. Autonomía CMS & Publicación No-Code",
    cs_usm_act2: "2. Telemetría & Analítica Digital",
    cs_usm_act3: "3. Calidad Funcional & Pruebas QA",
    cs_usm_st11: "1.1 Selección UI",
    cs_usm_st12: "1.2 Edición Drag&Drop",
    cs_usm_st13: "1.3 Pub. Strapi 5",
    cs_usm_st21: "2.1 Eventos GTM",
    cs_usm_st22: "2.2 Registro GA4",
    cs_usm_st23: "2.3 Heatmap Clarity",
    cs_usm_st31: "3.1 Despliegue Staging",
    cs_usm_st32: "3.2 Matriz QA Form",
    cs_usm_st33: "3.3 Pase a Prod",
    cs_usm_dt11: "Componentes visuales responsivos aprobados por UX.",
    cs_usm_dt12: "Editor visual Puck CMS con previsualización en vivo.",
    cs_usm_dt13: "API Headless Strapi 5 actualiza página sin redeploy dev.",
    cs_usm_dt21: "DataLayer & Triggers en GTM Debugger (`click_apply_now`).",
    cs_usm_dt22: "Métricas de conversión y dimensiones en GA4.",
    cs_usm_dt23: "Grabaciones de sesión anonimizadas en móviles.",
    cs_usm_dt31: "Publicación automática en ambiente Staging.",
    cs_usm_dt32: "Matriz de 15 casos de prueba funcionales.",
    cs_usm_dt33: "Firma de acta QA e hito de despliegue a producción.",
    ...
    cs_usm_b101_desc: "<strong>Como Product Owner</strong>, quiero maquetar y publicar landing pages dinámicas con Puck CMS conectado a Strapi 5 sin depender de sprints de desarrollo.",
    cs_usm_b101_ac: "<strong>Criterios de Aceptación (BDD):</strong><br>• <i>Given:</i> El usuario está autenticado en el editor visual de Puck CMS.<br>• <i>When:</i> Arrastra los componentes corporativos y presiona 'Publicar en Strapi 5'.<br>• <i>Then:</i> La página se despliega en producción en vivo en menos de 15 minutos.",
    cs_usm_b102_desc: "<strong>Como UX Designer & PO</strong>, quiero validar maquetaciones en alambre creadas con Firebase AI antes de pasarlas a Strapi 5.",
    cs_usm_b102_ac: "<strong>Criterios de Aceptación (BDD):</strong><br>• <i>Given:</i> El mockup en Firebase AI cuenta con V°B° del equipo comercial.<br>• <i>When:</i> Se exportan los tokens de diseño al CMS.<br>• <i>Then:</i> Los componentes se previsualizan 100% responsive en móvil y desktop.",
    cs_usm_b201_desc: "<strong>Como Business Analyst</strong>, quiero configurar triggers en GTM para capturar la intención de solicitud en GA4.",
    cs_usm_b201_ac: "<strong>Criterios de Aceptación (BDD):</strong><br>• <i>Given:</i> El usuario navega en la landing pública de producto.<br>• <i>When:</i> Hace clic en el botón `click_apply_now`.<br>• <i>Then:</i> GTM dispara el evento a GA4 registrando la sesión y dispositivo.",
    cs_usm_b202_desc: "<strong>Como Product Owner</strong>, quiero integrar Microsoft Clarity para grabar sesiones anonimizadas y detectar fricción en móviles.",
    cs_usm_b202_ac: "<strong>Criterios de Aceptación (BDD):</strong><br>• <i>Given:</i> El script de Clarity está activo en el tag global de GTM.<br>• <i>When:</i> Los usuarios interactúan en smartphones.<br>• <i>Then:</i> Se generan mapas de calor semanales identficando clics muertos.",
    cs_usm_b301_desc: "<strong>Como Business Analyst</strong>, quiero ejecutar la matriz de Pruebas QA en Staging antes de dar el V°B° de pase a producción.",
    cs_usm_b301_ac: "<strong>Criterios de Aceptación (BDD):</strong><br>• <i>Given:</i> La landing creada en Puck CMS está publicada en ambiente Staging.<br>• <i>When:</i> Se verifica el 100% de la matriz de casos de prueba de formularios.<br>• <i>Then:</i> Se firma el acta de certificación funcional QA para habilitar el despliegue.",
    cs_usm_b302_desc: "<strong>Como Product Owner</strong>, quiero un mecanismo de bloqueo que impida el pase a producción si se detectan errores bloqueantes.",
    cs_usm_b302_ac: "<strong>Criterios de Aceptación (BDD):</strong><br>• <i>Given:</i> Un caso de prueba funcional falla en la matriz de QA.<br>• <i>When:</i> Se registra la incidencia en Jira con severidad Alta.<br>• <i>Then:</i> El pipeline retiene el despliegue y notifica a la triada.",
